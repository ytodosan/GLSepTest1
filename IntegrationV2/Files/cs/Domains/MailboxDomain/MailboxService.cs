	...
	public class MailboxService : IMailboxService
	{
		...
		public Mailbox GetMailboxBySenderEmailAddress(string senderEmailAddress, bool userMailboxesOnly = true,
			...
			return userMailboxesOnly
				? allMailboxes.FirstOrDefault(
					m => m.SenderEmailAddress.Equals(senderEmailAddress, StringComparison.OrdinalIgnoreCase) &&
		...
		/// <inheritdoc cref="IMailboxService.GetEmails(Guid)"/>
		public List<string> GetEmails(Guid mailboxId) {
			return _mailboxRepository.GetEmails(mailboxId).ToList();
		}
