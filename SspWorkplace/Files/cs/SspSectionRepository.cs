namespace Terrasoft.Configuration.Section
{
	...
	public class SspSectionRepository : SectionRepository
	{
		...
		protected EntitySchemaManager SystemEntitySchemaManager;
		...
		protected List<string> _allowedSystemSchemas;
		protected virtual List<string> AllowedSystemSchemas {
			get {
				return _allowedSystemSchemas;
			}
			set {
				_allowedSystemSchemas = value;
			}
		} 
		...
		public SspSectionRepository(UserConnection uc) : base(uc) {
			...
			SystemEntitySchemaManager = UserConnection.SystemEntitySchemaManager;
			AllowedSystemSchemas = new List<string> { "Contact", "Account" };
		}
		...
		private IEnumerable<Guid> GetSectionNonAdministratedByRecordsEntityIds(Section section) {
			...
			var result = GetNonAdministratedByRecordsEntityUIds(moduleEntityIds);
		...
		private bool CheckIfSchemaAllowed(Guid entityUId, string schemaName) {
			return AllowedSystemSchemas.Contains(schemaName) || SystemEntitySchemaManager.FindItemByUId(entityUId) == null;
		}
		...
		/// <summary>
		/// Returns not administrated by records <see cref="Entity"/> identifier collection from <paramref name="entityUIds"/>.
		/// </summary>
		/// <param name="entityUIds"><see cref="Entity"/> identifier collection.</param>
		/// <returns><see cref="Entity"/> identifiers collection.</returns>
		protected IEnumerable<Guid> GetNonAdministratedByRecordsEntityUIds(IEnumerable<Guid> entityUIds) {
			var watch = Stopwatch.StartNew();
			LogDebugStart("GetNonAdministratedByRecordsEntityIds", watch);
			var result = new List<Guid>();
			foreach (var entityUId in entityUIds) {
				LogDebug($"GetNonAdministratedByRecordsEntityUIds start foreach EntitySchemaManager.GetItemByUId {entityUId}", watch);
				var schema = EntitySchemaManager.GetItemByUId(entityUId);
				LogDebug($"GetNonAdministratedByRecordsEntityUIds end foreach EntitySchemaManager.GetItemByUId {entityUId}", watch);
				LogDebug($"GetNonAdministratedByRecordsEntityUIds start foreach CheckIfSchemaAllowed {entityUId}, {schema.Name}", watch);
				if (CheckIfSchemaAllowed(entityUId, schema.Name)) {
					LogDebug($"GetNonAdministratedByRecordsEntityUIds end foreach CheckIfSchemaAllowed  {entityUId}, {schema.Name}", watch);
					LogDebug($"GetNonAdministratedByRecordsEntityUIds start dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
					var schemaIsAdministratedByRecords = _dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords(schema.Name);
					LogDebug($"GetNonAdministratedByRecordsEntityUIds end dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
					if (!schemaIsAdministratedByRecords) {
						result.AddIfNotExists(entityUId);
					}
				}
				else {
					LogDebug($"GetNonAdministratedByRecordsEntityUIds foreach CheckIfSchemaAllowed {entityUId}, {schema.Name} found SystemSchema", watch);
				}
			}
			LogDebugEnd("GetNonAdministratedByRecordsEntityUIds", watch);
			return result;
		}

		/// <summary>
		/// Returns not administrated by operations <see cref="Entity"/> identifier collection from <paramref name="entityUIds"/>.
		/// </summary>
		/// <param name="entityUIds"><see cref="Entity"/> identifier collection.</param>
		/// <returns><see cref="Entity"/> identifiers collection.</returns>
		protected IEnumerable<Guid> GetNonAdministratedByOperationsEntityUIds(IEnumerable<Guid> entityUIds) {
			var watch = Stopwatch.StartNew();
			LogDebugStart("GetNonAdministratedByOperationsEntityUIds", watch);
			var result = new List<Guid>();
			foreach (var entityUId in entityUIds) {
				LogDebug($"GetNonAdministratedByOperationsEntityUIds start foreach EntitySchemaManager.GetItemByUId {entityUId}", watch);
				var schema = EntitySchemaManager.GetItemByUId(entityUId);
				LogDebug($"GetNonAdministratedByOperationsEntityUIds end foreach EntitySchemaManager.GetItemByUId {entityUId}", watch);
				LogDebug($"GetNonAdministratedByOperationsEntityUIds strart foreach CheckIfSchemaAllowed {entityUId}, {schema.Name}", watch);
				if (CheckIfSchemaAllowed(entityUId, schema.Name)) {
					LogDebug($"GetNonAdministratedByOperationsEntityUIds end foreach CheckIfSchemaAllowed {entityUId}, {schema.Name}", watch);
					LogDebug($"GetNonAdministratedByOperationsEntityUIds start dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
					var schemaIsAdministratedByOperations = _dbSecurityEngine.GetIsEntitySchemaAdministratedByOperations(schema.Name);
					LogDebug($"GetNonAdministratedByOperationsEntityUIds end dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
					if (!schemaIsAdministratedByOperations) {
						result.AddIfNotExists(entityUId);
					}
				} else {
					LogDebug($"GetNonAdministratedByOperationsEntityIds foreach CheckIfSchemaAllowed {entityUId}, {schema.Name} found SystemSchema", watch);
				}
			}
